...
export class HttpClient {
  ...
  public async request<T>(options: HttpRequestOptions): Promise<CoffeeMailResponse<T>> {
    ...
    try {
      ...
      if (!response.ok) {
        const error = createErrorFromResponse(response.status, responsePayload, this.locale);
      ...
      if (err instanceof Error && err.name === 'AbortError') {
        const error: CoffeeMailError = new NetworkError(
          getI18nMessage('requestTimeout', this.locale, { timeoutMs: this.timeoutMs })
        );
        return { data: null, error };
      }
